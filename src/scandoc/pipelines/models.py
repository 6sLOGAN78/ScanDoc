...
class PipelineConfig(BaseModel):
    ...
    routing_mode: str = Field("adaptive", description="Agentic routing strategy ('adaptive', 'fast', 'deep', 'fallback')")
    ocr_model: Optional[str] = Field(None, description="Specific OCR model to use")
    layout_model: Optional[str] = Field(None, description="Specific Layout model to use")
    table_model: Optional[str] = Field(None, description="Specific Table model to use")
    formula_model: Optional[str] = Field(None, description="Specific Formula model to use")
    confidence_threshold: float = Field(0.85, ge=0.0, le=1.0, description="Confidence threshold for fast-path vs OCR fallback")
